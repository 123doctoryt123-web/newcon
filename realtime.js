...
if (status === "CHANNEL_ERROR" || status === "TIMED_OUT") {          // حاول تاني بعد 3 ثواني
  ...
  // ============================================================
  // 10) القائد — تحديث أعضاء الفريق والتوقعات والتحديات
  // ============================================================
  if (page === "leader.html") {
    subscribeWithRetry("rt-leader-members", function (channel) {
      channel.on("postgres_changes", {
        event: "*", schema: "public", table: "members"
      }, function () {
        if (typeof loadTeamMembers === "function") loadTeamMembers();
        if (typeof loadAllTeamsRank === "function") loadAllTeamsRank();
      });
    });
    subscribeWithRetry("rt-leader-predictions", function (channel) {
      channel.on("postgres_changes", {
        event: "*", schema: "public", table: "predictions"
      }, function () {
        if (typeof loadTeamPredictions === "function") loadTeamPredictions();
      });
    });
    subscribeWithRetry("rt-leader-challenges2", function (channel) {
      channel
        .on("postgres_changes", { event: "*", schema: "public", table: "team_challenges" }, function () {
          if (typeof loadTeamChallenges === "function") loadTeamChallenges();
        })
  ...
  // ============================================================
  // 11) لوحة المتصدرين — تحديث النقاط
  // ============================================================
  if (page === "leaderboard.html") {
    subscribeWithRetry("rt-leaderboard-members", function (channel) {
      channel.on("postgres_changes", {
        event: "UPDATE", schema: "public", table: "members"
      }, function () {
  ...
  // ============================================================
  // 12) التوقعات المؤتمرية — تحديث الإعدادات
  // ============================================================
  if (page === "confpredict.html") {
    subscribeWithRetry("rt-confpredict-settings", function (channel) {
      channel.on("postgres_changes", {
        event: "*", schema: "public", table: "settings"
      }, function () {
        if (typeof init === "function") init();
  ...
  // ============================================================
  // 13) الأحجية — تحديث تلقائي لو الأدمن غيّر الأحجية
  // ============================================================
  if (page === "puzzle.html") {
    subscribeWithRetry("rt-puzzle", function (channel) {
      channel.on("postgres_changes", {
        event: "*", schema: "public", table: "puzzles"
      }, function () {
        if (typeof loadPuzzle === "function") loadPuzzle();
      });
    });
  }

  // ============================================================
  // 14) العجلة — تحديث الحالة
  // ============================================================
  if (page === "wheel.html") {
    subscribeWithRetry("rt-wheel-settings", function (channel) {
      channel.on("postgres_changes", {
        event: "*", schema: "public", table: "settings"
      }, function () {
  ...
  // ============================================================
  // 15) السؤال اليومي — تحديث تلقائي (مع retry)
  // ============================================================
  if (page === "daily-question.html") {
    subscribeWithRetry("rt-daily-question-v2", function (channel) {
      channel.on("postgres_changes", {
  ...
  // ============================================================
  // 16) الاستفتاء — تحديث الأصوات والاستفتاءات (مع retry)
  // ============================================================
  if (page === "poll.html") {
    subscribeWithRetry("rt-poll-votes-v2", function (channel) {
      channel
        .on("postgres_changes", { event: "*", schema: "public", table: "poll_votes" }, function () {
          if (typeof reloadVotesOnly === "function") reloadVotesOnly();
        })
  ...
  // ============================================================
  // 17) جدار الامتنان — تحديث تلقائي (مع retry)
  // ============================================================
  if (page === "gratitude.html") {
    subscribeWithRetry("rt-gratitude-v2", function (channel) {
      channel
        .on("postgres_changes", { event: "INSERT", schema: "public", table: "gratitude_wall" }, function () {
          if (typeof loadWall === "function") loadWall();
        })
        .on("postgres_changes", { event: "UPDATE", schema: "public", table: "gratitude_wall" }, function () {
          if (typeof loadWall === "function") loadWall();
        });
    });
  }
  ...
  // ============================================================
  // 18) صندوق الأسرار — تحديث تلقائي (مع retry)
  // ============================================================
  if (page === "secretbox.html") {
    subscribeWithRetry("rt-secrets-v2", function (channel) {
      channel
        .on("postgres_changes", { event: "INSERT", schema: "public", table: "secret_box" }, function () {
          if (typeof loadSecrets === "function") loadSecrets();
        })
        .on("postgres_changes", { event: "UPDATE", schema: "public", table: "secret_box" }, function () {
          if (typeof loadSecrets === "function") loadSecrets();
        });
    });
  }
  ...
  document.addEventListener("visibilitychange", function () {
    if (document.visibilityState === "visible") {
      ...
      if (page === "dashboard.html" && typeof loadLatestAnnouncement === "function") loadLatestAnnouncement();
      if (page === "admin.html" && typeof loadCurrentAnnouncement === "function") loadCurrentAnnouncement();
      if (page === "leader.html") {
        if (typeof loadTeamMembers === "function") loadTeamMembers();
        if (typeof loadAllTeamsRank === "function") loadAllTeamsRank();
      }
      if (page === "leaderboard.html" && typeof loadAndRender === "function") loadAndRender();
      if (page === "daily-question.html" && typeof loadQuestion === "function") loadQuestion();
      if (page === "poll.html" && typeof loadPoll === "function") loadPoll();
      if (page === "gratitude.html" && typeof loadWall === "function") loadWall();
      if (page === "secretbox.html" && typeof loadSecrets === "function") loadSecrets();
      if (page === "wheel.html" && typeof loadStatus === "function") loadStatus();
      if (page === "puzzle.html" && typeof loadPuzzle === "function") loadPuzzle();
      if (page === "confpredict.html" && typeof init === "function") init();
    }
  });
