...
// ============================================================
// أسئلة مجهولة — درس الكتاب
// ============================================================
async function loadAdminBQ(){
  var feed = document.getElementById('adminBqFeed');
  var countEl = document.getElementById('adminBqCount');
  if(!feed) return;
  var res = await supabase.rpc('get_book_questions');
  if(res.error || !res.data || !res.data.length){
    if(countEl) countEl.textContent = '0 سؤال';
    feed.innerHTML = '<div class="empty-state">مفيش أسئلة لسه</div>';
    return;
  }
  if(countEl) countEl.textContent = res.data.length + ' سؤال';
  feed.innerHTML = res.data.map(function(q){
    var d = new Date(q.created_at);
    var timeStr = d.toLocaleString('ar-EG',{day:'numeric',month:'short',hour:'2-digit',minute:'2-digit'});
    return '<div class="submission-item" id="adminBqItem-'+q.id+'" style="align-items:flex-start;gap:12px">'
         + '<div style="flex:1"><div style="font-size:14px;color:var(--mist);line-height:1.6;margin-bottom:4px">❓ ' + escHtml(q.question) + '</div>'
         + '<div class="meta">' + timeStr + '</div></div>'
         + '<button class="btn danger small" style="flex-shrink:0;margin-top:2px" onclick="adminDeleteBQ('+q.id+')">🗑️</button>'
         + '</div>';
  }).join('');
}

async function adminDeleteBQ(id){
  if(!confirm('تحذف السؤال ده؟')) return;
  var member = getSession();
  if(!member) return;
  var res = await supabase.rpc('admin_delete_book_question', { p_id: id, p_admin_id: member.id });
  if(res.error){ alert('❌ مش قادر يحذف'); return; }
  var el = document.getElementById('adminBqItem-' + id);
  if(el){ el.style.opacity='0'; el.style.transition='opacity .25s'; setTimeout(function(){ el.remove(); loadAdminBQ(); }, 280); }
}

async function adminClearAllBQ(){
  if(!confirm('هتمسح كل الأسئلة؟ مفيش رجعة!')) return;
  var member = getSession();
  if(!member) return;
  var res = await supabase.rpc('admin_clear_book_questions', { p_admin_id: member.id });
  if(res.error){ alert('❌ مش قادر يمسح'); return; }
  loadAdminBQ();
}

// تحميل الأسئلة لما تاب الكتاب يتفتح
(function watchBookTab(){
  var panel = document.getElementById('panel-book');
  if(!panel) return;
  var observer = new MutationObserver(function(){
    if(panel.classList.contains('active')) loadAdminBQ();
  });
  observer.observe(panel, { attributes: true, attributeFilter: ['class'] });
})();
